<?php
    ...
    if(isset($_POST['btn_login'])){
        ...
        $password = md5($pass);
        $login = "SELECT * FROM users WHERE username = '$user' AND password = '$password' AND  permission = '1'";
        $querry = mysqli_query($conn, $login);
        $num_rows_login = mysqli_num_rows($querry);
        if($num_rows_login == 0){
            echo "<script>window.alert('Sai thông tin đăng nhập !');</script>";
        }else{
            while($data = mysqli_fetch_array($querry)){
                $_SESSION['user_id'] = $data['id'];
                $_SESSION['user_admin'] = $data['username'];
                $_SESSION['pass_admin'] = $data['password'];
                $_SESSION['email_admin'] = $data['gmail'];
                $_SESSION['fullname_admin'] = $data['fullname'];
                $_SESSION['sdt_admin'] = $data['phone'];
            }
            echo "<script>window.alert('Đăng nhập hệ thống thành công !');window.location = 'index.php'</script>";
...
<html lang="en">
    <head>
        <meta charset="utf-8">
        <title>Onplaza - Đăng nhập hệ thống</title>
