...
if (isset($_POST['sbm']) && !empty($_POST['search'])) {
    $search = $_POST['search'];
    $query =  mysqli_query($conn, "SELECT * FROM cart INNER JOIN status ON cart.id_status = status.id_status WHERE  cart.id_status = 3 AND name_kh LIKE '%$search%';");
    $total_prd = mysqli_num_rows($query);
} else {
    $query = mysqli_query($conn, "SELECT * FROM `cart` INNER JOIN status ON cart.id_status =status.id_status WHERE  cart.id_status = 3;");
}
...
<head>
    <meta charset="utf-8">
    <title>Onplaza - Danh sách đơn hàng hoàn thành</title>
