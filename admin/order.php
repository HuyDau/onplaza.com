...
if (isset($_POST['sbm']) && !empty($_POST['search'])) {
    $search = $_POST['search'];
    $query = mysqli_query($conn, "SELECT * FROM cart INNER JOIN status ON cart.id_status = status.id_status WHERE cart.id_status = 1 OR cart.id_status = 2 AND name_kh LIKE '%$search%'  ;");
    $total_prd = mysqli_num_rows($query);
} else {
    $query = mysqli_query($conn, "SELECT * FROM `cart` INNER JOIN status ON cart.id_status =status.id_status WHERE cart.id_status = 1 OR cart.id_status = 2;");
}
                                        ...
                                        if (!empty($query)) {
                                            ...
                                            while ($row = mysqli_fetch_array($query)) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="detail_order.php?id=<?= $row['id_cart'] ?>"><span
                                                                class="badge badge-light-pink">Detail</span></a>
                                                    </td>
                                                        ...
                                                        <div class="btn-group dropdown">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                <a class="dropdown-item"
                                                                    href="confirm_order.php?id=<?= $row['id_cart'] ?>"><i
                                                                        class="mdi mdi-pencil mr-2 text-muted font-18 vertical-middle"></i>Confirm</a>
                                                                <a class="dropdown-item"
                                                                    href="complete_order.php?id=<?= $row['id_cart'] ?>"><i
                                                                        class="mdi mdi-check-all mr-2 text-muted font-18 vertical-middle"></i>Complete</a>
                                                                <a class="dropdown-item"
                                                                    href="remove_order.php?id=<?= $row['id_cart'] ?>"><i
